...
<#
.SYNOPSIS
    Build and push World War Game Docker image to GitHub Container Registry

.DESCRIPTION
    Builds the World War Game React application Docker image and optionally pushes to GitHub Container Registry

.PARAMETER GitHub Container RegistryUsername
    GitHub Container Registry username (default: suparious)

.PARAMETER ImageName
    Docker image name (default: world-war-game)
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$ImageName = "world-war-game",
...
Write-ColorOutput "`n=== World War Game - Docker Image Builder ===" $colors.Header
Write-ColorOutput "Building Docker image for strategic board game`n" $colors.Info
...
Write-ColorOutput "This may take 2-5 minutes" $colors.Info
Write-Host ""
...
docker run --rm $fullImageName nginx -t
if ($LASTEXITCODE -ne 0) {
    Write-ColorOutput "⚠️  Image test failed, but build succeeded" $colors.Warning
}
else {
    Write-ColorOutput "✅ Nginx configuration valid" $colors.Success
}
Write-Host ""
...
if (-not $Push) {
    ...
    Write-Host "     docker run --rm -p 8080:80 $fullImageName" -ForegroundColor Yellow
